...
#Resume=>Machine Learning Inference step
    ...
    response = lambda_client.invoke(
        FunctionName = "arn:aws:lambda:us-west-1:319163603467:function:resumeAI-devbackend",
        InvocationType = "RequestResponse",
        Payload = json.dumps(event, indent=4)
    )
    response = json.loads(response['Payload'].read().decode('utf-8')) #call lambda function resumeAI
    print(response)
    if (response["statusCode"] == 500):
        ...
        inferenced_data = response["profile"]
